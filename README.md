...
Com essas questões, foi feita uma rede neural anteriormente para a predição dos atributos de saída (valor da energia total e de formação da nanopartícula). Entretanto, foi analisado que as métricas RMSE (Root Mean Square Error) dessa rede neurais não estavam bons, informando que a performance deste modelo de redes neurais não está preciso o suficiente.

Uma forma de melhorar estes parâmetros é fazendo uma otimização dos hiperparâmetros do modelo. A ciência que pode ser utilizada para esta problemática são ao Algorítmos Genéticos..... português, perceptron multicamadas, que é uma rede neural artificial moderna de alimentação direta (feedforward). Essa rede é composta por várias camadas, incluindo uma camada de entrada, uma ou mais camadas ocultas e uma camada de saída.[5]
